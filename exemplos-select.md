...
```sql
-- Sim o * selecionas todas as tabelas sem exceção
```
...
O `WHERE` permite determinar **quias registros devem aparecer** no resultado. Na prática, são condições para execução do `SELECT`.
...
Exibir os produtos que custem menos de 500 e quantidades acima de 20.
...
Exibir os produtos que custem mais que 3000 ou com quantidade zerada.
...
**Obs.:** o uso do `NOT` não é obrigtório, desde que tu/você consiga o mesmo resultado usando uma lógica diferente, como no exemplo:
...
```sql
SELECT * FROM produtos WHERE fornecedor_id IN (1, 4, 8); -- lista de valores
```
...
`LIKE` é usado principalmentepara realizar pesquisas em texto. Junto com o caractere `%` permite fazer buscas baseadas em partes de uma string.
...
```sql
SELECT DISTINCT fornecedor_id FROM produtos WHERE preco > 500;
```
...
**ATENÇÃO:** não coloque espaço entre o nome da função e o parênteses!

## Recursos de agrupamento

`GROUP BY` reúne registros que possuem determinado valor em comum.
...
```sql
SELECT fornecedor_id, COUNT(*) AS total_produto FROM produtos GROUP BY fornecedor_id;
```
...
```sql
SELECT fornecedor_id, ROUND(AVG(preco), 2) AS preco_medio FROM produtos GROUP BY fornecedor_id;
```
...
Exemplo: mostrar somente os fornecedores que possuem pelomenos dois produtos cadastrados.

```sql
SELECT fornecedor_id, COUNT(*) AS total_produtos FROM produtos GROUP BY fornecedor_id HAVING COUNT(*) >= 2;
```
